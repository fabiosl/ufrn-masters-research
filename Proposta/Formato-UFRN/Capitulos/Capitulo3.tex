...
Filtering posts at a database level is represented by a simple SQL query, as illustrated on Listing~\ref{filter-posts-query-example}.
...
On the last section, a set of improvements were made to the posts table, but some operation SLAs remained broken. On this section we discuss how changes on the app/database infrastructure could fix the broken SLAs.

\subsubsection{Changes on tags structure}
To solve the problem of querying and updating posts that have a set of tags, we have proposed a separate table to store tags information. . 

On Figure~\ref{fig:tagTable} is presented the elements that compose a \textit{tag} record. Each tag has a unique ID - \textit{idtags}, a \textit{post\_id} (foreign key to the post table), a \textit{tag\_user} (the username of the user who has tagged this post) and \textit{tag\_name}, the subject of this post. 
...
Indexes were added to the post\_id, tag\_user and tag\_name columns. With the proposed table structure, adding and removing tags from a set of posts that have other tags needed new query fomats:
...
DELETE FROM tags where (tag_user="user1" AND tag_name="tag1")

\end{lstlisting} 

The query time was, however, still very high, and a post\_id\_user\_name\_tag\_name index was created. This way, the \textit{Classify posts (add/remove tags)} operation was ok, as FigureX suggests. 
...
dizer que o mapeamento foi feito de acordo com esse data format: https://www.dropbox.com/s/jb3w2o76lim070s/Screenshot%202016-01-21%2018.51.13.png?dl=0
