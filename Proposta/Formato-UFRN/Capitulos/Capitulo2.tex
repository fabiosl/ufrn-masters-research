...
Nessa secao nos detalhamos o problema que atacamos nessa pesquisa de mestrado. Um bom entendimento das secoes anteriores e do mapeamento sistematico disponivel em \cite{fabioMartinSM} eh requerida.
...
Quando se esta desenvolvendo uma app a principio nem sempre eh possivel usar a tecnologia mais apropriada para aquela aplicacao. Isso se deve em grande aprte, a, como Martin Fowler dizia, nao eh possivel saber de antemao todos os requisitos de uma aplicacao.

Utilizar diversos tipos de dbs logo de inicio numa aplicacao pode ser muito custoso em termos de desenvolvimento e tambem pode causar uma overengineering muito grande.

Todo esse cenario leva, em algumas vezes, a cenarios de transicoes de bancos de dados, e com o conceito de Polyglot persistence essas decisoes sao cada vez mais comuns. Esse cenario e tao comum atualmente que existem empresas especializadas em consultoria de transicoes e hospedagem de novas arquiteturas de DB, como o instaclustr.

Essas transicoes sao necessarias, na maioria das veze, pois nem sempre os bancos mais apropriados para uma determinada tarefa sao utilizados de antemao. 

Ha ainda os os casos onde uma tecnologia nao existia quando se estava desenvolvendo uma aplicacao e passa a existir depois. Tome o caso do Twitter, por exemplo... Toda a estrutura de DB do twitter foi desenvolvida em cima do MySQL. Depois construiram e migraram para um FlockDB, um BD especifico para Grafos. 

Por outro lado, processos de transicoes de bancos de dados sao custosos e a industria nao segue um padrao na hora de fazer replace de um banco em detrimento de outros. Isso pode gerar prejuizo, perda de tempo e ate mesmo uma rollback na transicao, cenario onde uma aplicacao eh transicioanada e depois passam a voltar a usar uma outra tecnologia. 

Isso foi constatado em \cite{fabioMartinSM}, um mapeamento sistematico que revelou, por exemplo, que transicoes de BDs seguem procedimentos nao padroes e podem variar significativamente entre aplicacoes. 
