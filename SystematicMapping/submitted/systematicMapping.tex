...
\author{Fabio Leal$^{\rm a}$ and Martin A. Musicante$^{\ast}$\thanks{$^\ast$Corresponding author. Email: sousaleal.fabio@gmail.com, mam@dimap.ufrn.br .
...
\citep{6546068} proposes SYBL: \textit{An Extensible Language for Controlling Elasticity in Cloud Applications. SYBL allows specifying in detail elasticity monitoring, constraints, and strategies at different levels of cloud applications, including the whole application, application component, and within application component code.} In other words, SYBL can also be seen as a language to specify SLAs in cloud environments.  

Some other initiatives that tried to define the mechanisms by which Webservice SLAs are established are WS-Agreement \citep{citeulike:2805191} and WSLA \citep{4578560}. Also, \citep{5547150} presents a framework for managing the mappings of the Low-level resource Metrics to High-level SLAs (LoM2HiS framework). 
...
\bibliography{tEISguide,systematicMapping}	
