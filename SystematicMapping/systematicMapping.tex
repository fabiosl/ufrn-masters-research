...
%describe how our systematic map was conceived. 
...
%The result of the screening the papers step.
...
\martin{A gente melhor conta o resultado\dots Falar do passo-a-passo do process muitas vezes confunde o leitor.}
...
\begin{description}
\item[RQ1)] What are the reasons to change from RDBMSs to NoSQL solutions?
  \begin{description}
  \item[AQ1.2)] What are the pros and cons to migrate from RDBMSs to NoSQL solutions?
  \item[AQ1.2)] How can we measure the overall improvements promised by this change?
  \end{description}
\item[RQ2)] How can SLAs be used to guide database transitioning processes from RDBMSs to NoSQL databases in cloud-based apps?
\item[RQ3)] Is there a standard representation of SLAs in cloud services?
\end{description}

\martin{2 questoes em relacao aas RQs\dots (1) Acho que precisa melhor explicar a motivacao por tras de cada uma delas (no paragrafo seguinte) e (2) Uma questao que vc responde mas nao pergunta é quais sao as tecnologias mais usadas para substituir/complementar a coisa relacional.}
...
Our questions are intended to cover \textit{why} it is good to migrate from RDBMSs to a NoSQL solution and \textit{how} we can do it. 
...
During this step, we also noticed that 2009 would be the starting year of our survey, since this was the year when the term NoSQL was reintroduced on the market, by Johan Oskarsson (Last.fm)\cite{ericevans}.

\subsection{Search Strategy}\label{sec:searchStrategy}

Google Scholar returned 74 results for our search string. We have also manually included other particularly relevant publications that were picked from selected sources, such as~\cite{mastersthesrilinda}. 
\martin{Por que citar esta? Tem outras? Quais?}
...
\martin{Algumas destas referencias nao dizem nada\dots colocar mais dados nelas ou tirar.}
