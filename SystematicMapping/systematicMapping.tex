...
In this context, the notion of Service-Level-Agreements (SLAs)~\cite{6253526} may be used as a parameter, to guide the process of choosing the most convenient technology.
\martin{Aqui precisaria motivar melhor a conveniencia de usar SLAs no processo de transicao.}
...
\martin{Aqui precisa de uma referencia para se apoiar.}
...
\martin{Terminar esta secao com algo como: In the next section we build a systematic mapping for\dots}
...
\martin{Aqui precisa aumentar e motivar mais a explicacao do que será feito na secao e, ja que tem mudancas, explicar \textit{como} as coisas sao feitas.}
...
\martin{OK... but: Why are these questions the good ones?}
...
We used ``\verb=changes AND database AND nosql AND sla AND cloud='' as a boolean search string on Google Scholar search engine. This resulted in a total of 88 publications.
\martin{Aqui precisa elaborar melhor: Por que apenas Google Scholar?, por exemplo...}


\subsection{Screening of papers}\label{sec:sop}
Inclusion and Exclusion criteria were used to filter the list of publications:
\martin{Daqui em diante, até o final da secao, o corpo de texto precisa ser \textbf{muito} incrementado. Todas as decisoes precisam ser justificadas e os passos precisam ser detalhados.}
...
\martin{Por que \textbf{apenas} 15? Isto precisa ser justificado, para mostrar que foi feito um trabalho serio... 
Quais sao esses trabalhos? Eles precisam estar referenciados e inseridos nos graficos para poder justificar a analise na secao das respostas aas RQ.}
...
\martin{OK a figura que mostra as facetas e divisoes dentro delas, mas precisariamos incluir os graficos tradicionais deste tipo de estudo, para comparar as tendencias dos dados. Como cada um dos valores de cada faceta se relaciona com os restantes?}

%\newpage
\section{Outcomes}
\label{sec:outcomes}

As mentioned in Section~\ref{sec:sop}, 15 publications were analyzed, and the answers to the research questions are presented below:
...
\martin{Hummm aqui temos um problema: Por que a sua busca nao achou este paper: ``Nadia Bennani, Chirine Ghedira Guegan, Martin A. Musicante, Genoveva Vargas-Solar: SLA-Guided Data Integration on Cloud Environments. IEEE CLOUD 2014: 934-935''?}
